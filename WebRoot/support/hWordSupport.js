...
(function(doc,win){
	if(isMobile()){
		...
		$("papers").onkeydown({target:$("papers").firstElementChild.firstElementChild});
	}
})(document, window);
function support(){
	...
	window.before_save=function(save_in_web){
		before_save();
		if(!save_in_web)$("saveInServie").style.display="none";
		...
		getJsessionid(
			...
			,function(jsessionid){
				myPost(
					...
					+"?method=localSaveUpload&JSESSIONID="+jsessionid+"&title="+$("title").innerText+".doc.html"
					,'<!doctype html>'+document.documentElement.outerHTML
					,function(res){
						if(res.isOk){
