...
-->
	...
	<body>
		...
		<div id="lists">
			<!--这是文件的下拉菜单-->
			<ul id="document_list" style="margin-left:202px;">
		...
		<ul id="ctrl" style="display:none;list-style:none;position:fixed;bottom:20px;right:20px;">
			<li id="cPREV">PREV</li><li id="cNEXT">NEXT</li><li id="cEXIT">EXIT</li>
		</ul>
		...
		<script id="callBacksOfClick" type="text/javascript">
			...
			$("fenYe").onmousedown=function(){return false;};
			...
			var time=0;//very important! 表示当前幻灯片的张数
			//init
			(function(){
			 ...
			 * 此方法用于获取当前PPT
			 */
			function getMyPPT(type){
				if(type=="getDes"){
					return $("fenYe").getElementsByTagName("ul")[0].getElementsByClassName(time+"")[0];
				}else if(type=="getDesUL"){
					return $("fenYe").getElementsByTagName("ul")[0];
				}else{
					return $("ppt").getElementsByClassName(time+"")[0];
				}
			}
			$("sideBar").onclick=function(event){
				...
				switch(target.id){
					case "addNew":
						...
						if(lastDiv!==false){
							...
							for(var i=0,rem=0;i<children.length;i++){
								try{
									if(rem==+children[i].className){children[i].className=rem+1}
					...
					case "delete":
						...
						ul.removeChild(getMyPPT("getDes"));
						var children=ul.childNodes;
						for(var i=0,rem=1;i<children.length;i++){
							try{
								if(rem!=children[i].className){
						...
						time--;
						myChangeColor(getMyPPT("getDes"));
						break;
					case "downwoard":
						var ppt=$("ppt");
						var div=ppt.getElementsByClassName(+time+1+"")[0];
						ppt.removeChild(div);
						ppt.insertBefore(div,getMyPPT());
						getMyPPT().className=+time+1+"";
						div.className=time+"";
						var ul=getMyPPT("getDesUL");
				...
				switch(target.parentNode.id){
					case "layout1":
						getMyPPT().innerHTML="<div class='title' contenteditable='true' style='text-align: center; width: 640px;height:60px;position: absolute;left: 50px;top:20px;font-size: 28px;'>单击此处添加标题</div>"
						...
						break;
					case "layout2":
						getMyPPT().innerHTML="<div class='title' contenteditable='true' style='width:640px;height:60px;text-align: center;position:absolute;left:50px;top:20px;font-size: 28px;'>单击此处添加标题</div>"
						+"<div class='content' contenteditable='true' style='width:640px;height:350px;position:absolute;top:115px;left:50px;'></div>";
						break;
					case "layout3":
						getMyPPT().innerHTML="<div class='title' contenteditable='true' style='text-align: center; width: 640px;height:60px;position:absolute;left:50px;top:70px;font-size: 28px;'>单击此处添加标题</div>";
						break;
					case "layout4":
						getMyPPT().innerHTML="<div class='title' contenteditable='true' style='text-align: center; width: 640px;height:60px;position:absolute;left:50px;top:20px;font-size: 28px;'>单击此处添加标题</div>"
			...
			function writeTitle(){
				var classNameOfParent=this.parentNode.className;
				...
				if(img||video){
					...
					var inn=this.innerText;
					inn=inn.replace(/\n/g," ");
					$("fenYe").getElementsByClassName(classNameOfParent)[0].innerText
						=/^[\w\s,.?~`\\/;:!@#$%\^<>。，？；、+\-&*()]+$/i.test(inn)
							?inn.length>22
								?inn.slice(0,22)+"..."
								:inn
							:inn.length>12?
								inn.replace(/\s/g,"").slice(0,10)+"..."
								:inn;
				}
				versionControl._prev._continue=false;
				this.onblur=null;
			}
			$("ppt").onclick=function(event){
				event=event||window.event;
				var target=getDiv(event.target||event.srcElement);
				if(target&&/\btitle\b/.test(target.className)){
					target.onblur=writeTitle;
				}else if(target){
			...
			var versionControl=new (function(){
				...
				,this.prev=function(event,arg0,arg1){
					arg0=arg0||"_prev"
					,arg1=arg1||"addNext";
					if(this[arg0].length==0)return;
					var rem=this[arg0].shift();
					if(!rem)return;
					switch(rem.type){
						case "#node":
							rem._node.innerHTML=rem.outer;
							if(/\bcontent\b|\btitle\b/.test(rem._node.className)){
								getMyPPT("getDesUL").onmousedown(
									{
										target:getMyPPT("getDesUL").getElementsByClassName(rem._node.parentNode.className)[0]
									}
								);
							}
							break;
				...
				,this.typeAdd=function(keyCode){
					var p=getDiv(getUserRange().startContainer);
					if(p.getElementsByTagName("img").length!=0)return;
					this.changePrev(
						{
							_node:p
							,type:"#node"
						...
						,!/[,。，、?？！!：:;；]$|[^0-9]\.$/.test(p.innerText)&&keyCode!=13
						,p.innerText==""||p.innerText=="\n"
					);
				}
				,this.styleChange=function(_node){
					try{
						if(!_node.getElementsByTagName("img")[0])
							this.addPrev(
								{
									_node:_node
									,type:"#node"
			...
			getMyPPT("getDesUL").onmousedown=function(event){
				event=event||window.event;
				var target=event.target||event.srcElement;
				...
				if(/li/i.test(target.tagName)){
					...
					time=className;
					if(!target.style.backgroundColor||target.style.backgroundColor=="inherit")myChangeColor(target);
				}else{
					return false;
				}
			};
			(function(){
				if($("ppt").childNodes.length>=1){
			...
			$("lists").onclick=function(event){
				...
				switch(target.id){
					...
					case "sub":
						...
						var userRange=getUserRange();
						insertHtmlAtCaret("<"+target.id+" style='font-size:"+(getFontSize(userRange.startContainer)/5*3)+"px;' contenteditable='false'>"+innertext+"</"+target.id+">");
						break;
					case "textArea":
			...
			function getDiv(target){
				try{
					while(target.nodeName=="#text"||!/\btitle\b|\bcontent\b/.test(target.className)){
		...
		<script id="callbacksOfKeydown" type="text/javascript">
			...
			function setStyle(type_str,setting_str,userRange,nodeType){
				...
				if(window.getSelection){
					if(!userRange){
						userRange=window.userRange||getUserRange();
						if(!userRange)return;
					}
					//版本控制
					versionControl.styleChange(userRange.startContainer);
