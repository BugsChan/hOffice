	...
	<body>
		...
		<div id="lists">
			...
			<ul id="document_list" style="margin-left:162px;">
				...
				<li id="saveInServie" style="display: block;">存为云文件</li>
				<li id="myDoc" style="display: block;">我的文件</li>
			</ul>
			<!--这是"文字"按钮的下拉菜单-->
			...
			<ul id="sum_list" style="margin: 0.5em 0px 0px 498px;">
				...
				<li title="expand(操作,拓展区域)">expand 拓展</li>
		...
		<div id="container_outer">
			<div id="container_inner">
				...
				%>
				<textarea id="table_input" style="position: absolute; display: block; border: 1px deepskyblue; top: 76px; left: 1411px; font-size: 15px; line-height: 15px; width: 65px; height: 14px; color: black;" placeholder="V1"></textarea>
			</div>
		</div>
		...
		<script id="callbacksOfClick" type="text/javascript" language="JavaScript">
			//根据列号返回数字,如"A"则返回1,"B"则返回2...
			//如果为数字,则返回字母
			function getLineCount(lineDes){
				var aNum="A".charCodeAt(0);
				if(typeof lineDes=="string"){
					var res=0,len=lineDes.length;
					lineDes=lineDes.toUpperCase();
					for(var i=0;i<len;i++){
						res+=(lineDes.charCodeAt(i)-aNum+1)*(Math.pow(26,len-i-1));
					}
				}else{
					var res="";
					...
					while(lineDes>0){
						...
						lineDes=parseInt(lineDes/26);
					}
				}
				return res;
			}
			function addLineCount(lineCount,isCut){
				var v=isCut?-1:1
				return getLineCount(getLineCount(lineCount)+v);
			}
			...
			(function(){
				...
				dragCell.lineCount=$("table").getEle("firstElementChild").getEle("firstElementChild").lastChild.innerText;
				dragCell.rowCount=+$("table").getEle("lastElementChild").getEle("lastElementChild").firstChild.innerText;
			})();
			...
			var userFuns={
				...
				checkLineCounts:function(smaller,bigger){
					smaller=getLineCount(smaller);
					bigger=getLineCount(bigger);
					return smaller<=bigger&&bigger<=getLineCount(dragCell.lineCount);
				},
					...
					try{
						...
						if((lineAdd&&rowAdd)||!(lineAdd||rowAdd)){
							...
							if(lineAdd){//纵向相加
								...
								var start=se[0].match(/[A-Z]+/).toString(),end=se[1].match(/[A-Z]+/).toString(),rowCount="r"+se[0].match(/[0-9]+/);
				...
				/**userFuns.EXPAND:用于拓展普通方法
				 * 此方法添加于2017-12-24,作用是为普通方法或操作提供拓展
				 * @param {Object} arr
				 * 	-0: 所需处理的字符串
				 * 	-1: 扩展区域
				 */
			...
			var userLeader={
				...
				,max:[
					...
					+"如原单元格中内容为: a32,则输入 1~3则系统将会按照 32 来进行比较(如您未看懂请直接点击'确定')"
				]
				,min:[
					userLeaderAtt.se,
					userLeaderAtt.kuozhan,
			...
			$("table").puttedIn=function(input){
				if(dragCell.inputTarget!==null&&!hasClassOf(dragCell.inputTarget,"free")){
					...
					}else if(hasClassOf(dragCell.inputTarget,"insertHTML")){
						dragCell.inputTarget.innerHTML=input.value;
					}else{
						if(input.value.indexOf("\n")>0||input.value.indexOf("\t")>0){
							insertTxtToCells(input.value);
						}else{
			...
			$("table").onclick=function(event){
				...
				if(/td/i.test(target.tagName)&&!/thead/i.test(target.parentNode.parentNode.tagName)
					...
					if($("chooserStyle")&&!hasClassOf(target,dragCell.chosedClassName)){
					...
					input.placeholder=target.className.match(/[A-Z]+/)+target.className.match(/[0-9]+/);
					var height;
			...
			function insertTxtToCells(res,it){
				res=res.replace(/\r/g,"");
				res=res.split(/\n/g);
				var maxLineCount=0;
				for(var i=0;i<res.length;i++){
					res[i]=res[i].split(/\t/g);
					maxLineCount=maxLineCount>res[i].length?maxLineCount:res[i].length;
				}
				it=it||dragCell.inputTarget;
				if(it){
					splite(it);
				}
				var initLineCount=getLineCount(getClassName(it,"[A-Z]+").toString())
				,initRowCount=+getClassName(it,"[0-9]+")
				,maxRowCount=initRowCount+res.length-1;
				maxLineCount+=initLineCount-1;
				if(!confirm("继续执行可能会损坏原有数据,是否继续执行?"))return;
				if(maxRowCount>dragCell.rowCount&&confirm("行数超过限制,是否自动增加?")){
					for(var i=dragCell.rowCount;i<maxRowCount;i++){
						addRow(true);
					}
				}
				if(maxLineCount>getLineCount(dragCell.lineCount)&&comfirm("列数超过限制,是否自动增加?")){
					for(var i=getLineCount(dragCell.lineCount);i<maxLineCount;i++){
						addLine(true);
					}
				}
				for(var i=0,_node;i<res.length;i++){
					for(var i2=0;i2<res[i].length;i2++){
						_node=getElementByClassNames(getLineCount(initLineCount+i2),"r"+(initRowCount+i));
						_node.innerText=res[i][i2];
						splite(_node);
					}
				}
				dragCell.inputTarget=null;
				...
				if(!dragCell.inputTarget.name&&dragCell.inputTarget.name!="null"){
					switch(keyCode){
						...
						case 39:
							...
							if(hasClassOf(dragCell.inputTarget,dragCell.lineCount))return;
			...
			function myParseFloat(str,isdown){
				var returning=parseFloat(str);
				if(!isNaN(+returning))return returning;
				if(str!="")return !isdown?Number.MIN_VALUE:Number.MAX_VALUE;
				return !isdown?Number.MAX_VALUE:Number.MIN_VALUE;
			}
			...
			$("lists").onclick=function(event){
				...
				switch(target.id){
					...
					case "save":
						...
						if(!confirm("你是否确认要写入文件?"))return;
						before_save();
						var blob = new Blob(['<!doctype html>'+document.documentElement.outerHTML], {type: "text/html;charset=utf-8"/*修改于2017-12-24,plain不妥,改为html*/});
					...
					case "addLine":
						...
						for(var i=0;i<tds.length;i++){
							...
							td.className=td.className.replace(/(\s+r[0-9]+)+/,getClassName(td," r[0-9]+")[0]);
							tds[i].parentNode.insertBefore(td,tds[i]);
							if(tds[i].rowSpan&&tds[i].rowSpan>1){
								var lc=getClassName(tds[i].nextSibling,"[A-Z]+")[0]
								,rc=+getClassName(tds[i],"[0-9]+")
								,lc2=addLineCount(getClassName(tds[i],"[A-Z]+")[0],true)
								,rs=tds[i].rowSpan;
								for(var i2=1,v,_lc=lc;i2<rs;i2++){
									v=getElementByClassNames(_lc,"r"+(rc+i2));
									while(v.rowSpan&&v.rowSpan>1&&getClassName(v,"r[0-9]+")[0]!="r"+(rc+i2)){
										_lc=addLineCount(_lc);
										if(getLineCount(_lc)>getLineCount(dragCell.lineCount)){
											v=null;
											break;
										}
										v=getElementByClassNames(_lc,"r"+(rc+i2));
									...
									td=document.createElement("td");
									td.className=lc2+" r"+(rc+i2);
									if(!v){
										var _node=document.getElementsByClassName("rowCount").getElementByInnerText((rc+i2)+"");
										_node.parentNode.appendChild(td);
									}else{
										v.parentNode.insertBefore(td,v);
									}
								}
							}
						}
					...
					case "addRow":
						...
						tds=table.getElementsByClassName("r"+rowCount);/*插入一行*/
						rowCount++;
						...
						for(var i="A",i2=0;i!=lastLineCount;i=addLineCount(i),i2++){
							if(tds.length>0&&tds[i2].rowSpan>1){//增加tes.length>0,为应对选中行为0的错误
								tds[i2].rowSpan++;
						...
						dragCell.rowCount++;//增加储存的行数
						$("insert_list").style.display="none";
						break;
					case "cutRow":
						var rowCount=dragCell.chosedClassName;
						if(dragCell.rowCount>1&&confirm("确认删除此行?")){
							if(rowCount==null||!/r[0-9]+/.test(rowCount)){
								try{
									rowCount=getClassName(dragCell.inputTarget,"r[0-9]+",true);
								}catch(e){
			...
			function sortRow(className,isdown){
				var limite=prompt("请输入行数范围,(冒号隔开的首行和末行,如'2:3')\n,如不输入默认从头排序,也可仅输入开头,如' 2 ' :");
				limite=limite.EUpper();
				if(!limite||limite==""){
					limite="r1:r"+dragCell.rowCount;
				}else if(/^[A-Z]*[0-9]+:$/.test(limite)){
					limite+="r"+dragCell.rowCount;
				}else if(/^[A-Z]*[0-9]+$/.test(limite)){
					limite+=":r"+dragCell.rowCount;
				}
				...
				try{
					...
					if(/([A-Z]*[0-9]+:[A-Z]*[0-9]+)?/.test(limite)){
						...
						if(confirm("注意:排序将可能会使一些单元格合并,任然确定排序?")){
							...
							while(start!=(end+1)){
								arr.push(myParseFloat(g("r"+start+className).innerText,isdown));
								start++;
							}
							//获取排列顺序数组
			...
			function sortLine(className,isdown){
				var limite=prompt("请输入列数范围,(冒号隔开的首列和末列,如'A:C')\n,不区分大小写(如不输入则默认从头排序),也可只写开头,如'B':")
				limite=limite.EUpper();
				if(!limite||limite==""){
					limite="A:"+dragCell.lineCount;
				}else if(/[A-Z]+[0-9]*:/.test(limite)){
					limite+=dragCell.lineCount;
				}else if(/[A-Z]+[0-9]*/.test(limite)){
					limite+=":"+dragCell.lineCount;
				}
				...
				try{
					...
					if(/([A-Z]+[0-9]*:[A-Z]+[0-9]*)?/.test(limite)){
						...
						if(confirm("注意:排序将可能会使一些单元格合并,任然确定排序?")){
							...
							while(start!=end){
								arr.push(myParseFloat(start.innerText,isdown));
								start=start.nextSibling;
							}
							arr.push(myParseFloat(end.innerText,isdown));
							//获取排列顺序数组
							arr=rankArr(arr,isdown);
							start=jd[0];
			...
			$("head").onclick=function(event){
				...
				switch(target.id){
					...
					case "requstExitFullScreen":
						var req=target.id=="requstFullScreen";
						myRequestFullScreen(req);
					...
					case "other":
					case "help":
						myChangeDisplay($(target.id+"_list"));
						break;
				}
			}
			...
			function addLine(isAdd){
				...
				if(isAdd){
					lineCount=addLineCount(lineCount);
					tr_in_head.insertCell(tr_in_head.cells.length-1).innerText=lineCount;
					for(var i=0;i<trs.length;i++){
						trs[i].insertCell(trs[i].cells.length-1).className=lineCount+" r"+(i+1);
					}
			...
			function getRelativePosition(obj,event,xy_str){
				if(xy_str==="x"){
					var x=$("container_outer").scrollLeft;
					x+=event.clientX-obj.offsetLeft;
					return x;
				}else if(xy_str==="y"){
					var y=$("container_outer").scrollTop;
