...
-->
	...
	<body>
		...
		<div id="lists" style="z-index: 1000;">
			...
			<ul id="count_list" style="margin: 1.5em 0px 0px 498px;">
			...
			<ul id="logic_list" style="margin: 2.5em 0px 0px 498px;">
			...
			<ul id="statistics_list" style="margin: 3.5em 0px 0px 498px;">
			...
			<ul id="math_list" style="margin: 5.5em 0px 0px 498px;">
				<li title="abs(判断位置,扩展区域)">abs 求绝对值</li>
				<li title="pow(底数,指数)">pow 幂函数</li>
			</ul>
			<ul id="funs_other_list" style="margin: 6.5em 0px 0px 498px;">
		...
		<script id="callbacksOfClick" type="text/javascript" language="JavaScript">
			...
			function search(searching_str, parentNode) {
				...
				if(parentNode.childNodes.length >= 1) {
					...
					var index, totleIndex = 0,
						str = parentNode.nodeValue;
			...
			var userFuns = {
				...
				computeSupport: function(arr, callBack) {
					try {
						...
						if((lineAdd && rowAdd) || !(lineAdd || rowAdd)) {
							...
							if(lineAdd) { //纵向相加
								var start = +se[0].match(/[0-9]+/),
									end = +se[1].match(/[0-9]+/),
									lineCount = se[0].match(/[A-Z]+/);
								...
								var start = se[0].match(/[A-Z]+/),
									end = se[1].match(/[A-Z]+/),
									rowCount = "r" + se[0].match(/[0-9]+/);
					...
					try {
						...
						if((lineCompare && rowCompare) || !(lineCompare || rowCompare)) {
							...
							if(lineCompare) {
								...
								var start = se[0].match(/[A-Z]+/).toString(),
									end = se[1].match(/[A-Z]+/),
									rowCount = +se[0].match(/[0-9]+/);
								if(!userFuns.checkLineCounts(start, end)) throw Error;
				...
				ifs: function(userTargetRange, rowArrange_on_only, pretreated_str, getInner, notToRes) {
					...
					if(lineArrange) { //纵向扩展
						var lineCount = dragCell.inputTarget.className.match(/[A-Z]+/),
							notAlert = false;
						targetEnd++;
						while(targetStart < targetEnd) {
							...
							notAlert = userFuns.putIn(getInner(myPretreated_str), notAlert, getElementByClassNames(lineCount, "r" + targetStart));
							targetStart++;
						}
					} else { //横向扩展
						var rowCount = dragCell.inputTarget.className.match(/[0-9]+/),
							notAlert = false;
						...
						while(targetStart != targetEnd) {
							...
							notAlert = userFuns.putIn(getInner(myPretreated_str), notAlert, getElementByClassNames(targetStart, "r" + rowCount));
						...
						function(ans, inner, j) {
							...
							if(userFuns.arg2[5][userFuns.arg2[6]]) {
								...
								userFuns.arg2[1] = userFuns.putIn(
									userFuns.arg1[userFuns.arg2[0]], userFuns.arg2[1], j ? getElementByClassNames(userFuns.arg2[4], "r" + userFuns.arg2[2]) : getElementByClassNames(userFuns.arg2[2], "r" + userFuns.arg2[4])
								);
							}
			...
			var userLeader = {
				...
				countifs: null,
				IF: [
					"请输入判断条件,条件中可以包含一个单元格位置,(例如: a1>3 ):", "请输入符合判断条件时的返回值(value_if_true),该返回值将会写入被选中的单元格中.", "请输入不符合判断条件时的返回值(value_if_false),该返回值将会写入被选中的单元格中.", userLeaderAtt.kuozhan
				],
				max: [
					userLeaderAtt.se,
					userLeaderAtt.kuozhan,
				...
				slice: [
					"请输入需要被分割的句子(如: 你好)或单元格位置(如: a1,不区分大小写):", "请输入开始分割的位置 (如: 0 ),如不输入则默认为0\n" +
					"!注意: 从0开始数,如 123 中1为第0位,2为第1位", "请输入分割结束的位置 (如: 3),仍如上一个参数一样从0开始数\n" +
					",但最后一位不会被包含在结果中", userLeaderAtt.kuozhan
				],
				average: [
					userLeaderAtt.se, userLeaderAtt.kuozhan
				],
				mode: [
					userLeaderAtt.se, userLeaderAtt.kuozhan
				],
				...
				abs: [
					"请输入数据 (如: -12) 或单元格位置 (如: a1):", userLeaderAtt.kuozhan
				],
				pow: [
					"请输入底数(可以为单元格位置,如: a1,不区分大小写):", '请输入指数(可以为单元格位置,如: a1,不区分大小写):'
				],
				...
				rank: [
					"请输入排名区域 (如: a1:a3 或 a1:c1),若整行(整列)都需要排序则只需输入行号或列号 (如 a 或 3)", "请输入排名方式,排序方式有down (降序) 或  up (升序) 两种,若不输入则按照降序处理"
				]
			}
				...
				if(!dragCell.inputTarget.name && dragCell.inputTarget.name != "null") {
					switch(keyCode) {
						...
						case 13:
							...
							if(!event.shiftKey || keyCode == 40) {
								if(hasClassOf(dragCell.inputTarget, dragCell.rowCount + "")) {
									addRow(true);
									$("table").onclick({
										target: getElement(getElement($("table"), "lastElementChild"), "lastElementChild").getElementsByClassName(getClassName(dragCell.inputTarget, "[A-Z]+"))[0]
									});
								} else {
						...
						case 39:
							...
							$("table").onclick({
								target: getElementByClassNames(lineCount, rowCount)
							});
							return false;
							break;
						case 37:
			...
			$("lists").onclick = function(event) {
				...
				switch(target.id) {
					...
					case "addRow":
						...
						var tr_to_insert;
						for(var i = 0; i < tds.length; i++) {
							if(+tds[i].innerText > rowCount) {
								tds[i].innerText = +tds[i].innerText + 1
							};
